...
class ElbPipe(Width:Int,GroupNum : Int) extends VldRdy(Width:Int){
    ...
    def BundleVec(Num:Int,Bund:Bundle) ={
    ...
    valid_mid = Wire(Vec(GroupNum +1 , Bool()))
    ready_mid = Wire(Vec(GroupNum +1 , Bool()))
    bits_mid  = Wire(Vec(GroupNum +1 , UInt(Width.W)))
    val DecoupledIO_mid = BundleVec((@@GroupNum+1)
    for(i<-0 until GroupNum){
